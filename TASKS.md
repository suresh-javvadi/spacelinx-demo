...
- [x] **T1.2** 🔴 **Apply the permission seed.** ✅ Done 2026-06-02 across all environments via the gated runner `database/migrations/seed/apply-permissions-seed.sh` (connectivity → read-only pre-flight → transactional ROLLBACK dry-run → confirm → apply → verify). Results: **dev +88 → 304 active; UAT +86 → 249 active; prod +85 → 249 active (= full catalog)**. The seed was hardened against the `UNIQUE(role_id, permission, deleted_at)` collision — confirmed real in **all three** environments (3 roles held both `VENDORS.DOC.DELETE` and `VENDORS.DOCUMENTS.DELETE`; the bare rename would have rolled the whole seed back). Typo grants/definition retired, **no capability lost** anywhere. **Follow-ups:** dev has ~56 active perms NOT in the catalog — review for catalog gaps/legacy; normalize `category_name` semantics (it holds the description) in the code-first migration (T2.2). *(Ref: `DB_FINDINGS.md`.)*
