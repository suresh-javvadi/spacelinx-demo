...
- [ ] **T1.2** 🔴 **Apply the permission seed** (review-then-run): `database/migrations/seed/permissions.seed.sql` (generated) inserts the **86** missing permissions (`ON CONFLICT DO NOTHING`, existing 163 untouched) and migrates the `VENDORS.DOC.DELETE` typo → `VENDORS.DOCUMENTS.DELETE` (incl. role grants). Run on dev → UAT → prod. Then normalize `category_name` semantics (it currently holds the description) in the code-first migration (T2.2). Acceptance: every UI-checked permission is grantable. **Must precede T1.4** (else those endpoints deny-all). *(Ref: `DB_FINDINGS.md`.)*
