CREATE OR REPLACE VIEW sc.grn_with_user_vw AS
SELECT 
    grn.id AS grn_id,
    grn.grn_number,
    ...
    CASE
        ...
        WHEN pt.name ILIKE '%po issued%' THEN po.order_date
   ...
   FROM 
     ...
     LEFT JOIN
        sc.purchase_order po ON grn.purchase_order_id = po.id
     LEFT JOIN
        sc.payment_term pt ON po.payment_term_id = pt.id
     LEFT JOIN
