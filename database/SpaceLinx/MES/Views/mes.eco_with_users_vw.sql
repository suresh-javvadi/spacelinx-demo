...
CREATE OR REPLACE VIEW mes.eco_with_users_vw
 AS
 SELECT eco.id,
    eco.number,
    eco.name,
    ...
    req_user.first_name || ' ' || req_user.last_name AS requestor_full_name,
    req_user.email AS requestor_email,

    -- ECO Approvers aggregated as JSON array
    json_agg(
        json_build_object(
            'approval_id', appr.id,
            ...
            'full_name', appr_user.first_name || ' ' || appr_user.last_name,
            'email', appr_user.email
        )
    ) FILTER (WHERE appr.id IS NOT NULL) AS approvers
FROM mes.eco eco
...
GROUP BY
    ...
    eco.planned_implementation_date, eco.approved_by, eco.approved_date,
    eco.status, eco.is_active, eco.created_at, eco.created_by, eco.updated_at, eco.updated_by,
    req_user.id, req_user.first_name, req_user.last_name, req_user.email;

ALTER TABLE mes.eco_with_users_vw
OWNER TO spacelinxuser;