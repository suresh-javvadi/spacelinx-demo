...
const PartHierarchy = () => {
  ...
  const [selectedPartId, setSelectedPartId] = useState(part_id || null);
  const [selectedPart, setSelectedPart] = useState(null);
  const [expandAll, setExpandAll] = useState(false);
  const [isDrawerOpen, setIsDrawerOpen] = useState(true);
  const [orientation, setOrientation] = useState("Horizontal");
  const [partIdToRedirect, setPartIdToRedirect] = useState("");
  const [partDetailsDrawerOpen, setPartDetailsDrawerOpen] = useState(false);
  ...
  const handleBackToCenter = useCallback(() => {
    ...
    const gVis = svg.select(".visualization-group");
  ...
  const fetchEntireHierarchyData = async (partId) => {
    ...
    try {
      const data = await fetchEntirePartHierarchy(partId);
  ...
  const drawTree = useCallback(
    (data, showIndicators) => {
      ...
      // Custom link generator to connect to the node border
      ...
      const isToggleVisible = (d) =>
        d.data.hasBom || d.children || d._children || d.isRecursive;
      ...
      const getToggleCircleFill = (d) => {
        if (d.isRecursive) return "#ff9800";
        return d.children ? "#444" : "#1976d2";
      };
      ...
      const getNodeContentHtml = (d) => {
        return `
          <div
            class="node-name part-link"
            title="${d.data.partNumber}"
          >
            ${d.data.partNumber || "N/A"}
          </div>
          <div class="node-pno">Name: ${d.data.name || "N/A"}</div>
          <div class="node-pno">Type: ${d.data.partTypeName || "N/A"}</div>
          <div class="node-pno">Manf. Part No: ${
            d.data.manufacturingPartNumber || "N/A"
          }</div>
          <div class="node-qty">Qty: ${d.data.quantity ?? "—"}</div>
        `;
      };
      ...
      const update = (source) => {
        ...
        const linkStartPosition = isHorizontal
        ...
        nodeEnter
          ...
          .attr("ry", 6)
          .attr("stroke", (d) =>
            showIndicators ? getPartTypeColor(d.data.partTypeName) : "#4a4a4a"
          );
  ...
  useEffect(() => {
    if (selectedPartId) {
      ...
      if (String(selectedPartId) !== part_id) {
        navigate(`/plm/parts/hierarchy/${selectedPartId}`, { replace: true });
      }
    } else {
      setHierarchyData(null);
      lastRootRef.current = null;
      ...
      setPartTypes([]);
      if (part_id) {
        navigate("/plm/parts/hierarchy", { replace: true });
      }
    }
  }, [selectedPartId, part_id, navigate]);
