...
import { fetchSubsystemViewBOM } from "../../../services/childPartService";
...
const filterTreeData = (data, term) => {
  ...
  for (const part of data) {
    if (matchesSearch(part, term)) {
      ...
      const filteredChildren = filterTreeData(part.children, term);
      if (filteredChildren.length > 0) {
        let partCopy = { ...part };
        partCopy.children = filteredChildren;
        filteredData.push(partCopy);
      }
    }
  }
...
const formatSubsystemTreeData = (data) => {
  if (!Array.isArray(data)) return [];

  return data.map((subsystem) => ({
    key: `subsystem/${subsystem.subsystemName}`,
    type: "SUBSYSTEM",
    name: subsystem.subsystemName,
    subsystemName: subsystem.subsystemName,
    children: formatPartChildren(
      subsystem.parts,
...
const SubsystemViewAntDTable = ({
  ...
  const initialAntDesignColumns = [
    ...
    },
    {
      ...
      className: "single-line-cell-wrapper",
      render: (_, record) => {
        if (record.type === "SUBSYSTEM") return null;
        return (
          <ion-icon
            name="trash-outline"
            onClick={async () => {
  ...
  const hideForSubsystem = (record) =>
    record?.type === "SUBSYSTEM" ? { props: { colSpan: 0 } } : null;
  ...
  const fetchSubsystemViewBOMTreeData = async (selectedId) => {
    ...
    try {
      const data = await fetchSubsystemViewBOM(selectedId);
      if (data) {
        const formattedData = formatSubsystemTreeData(data);
...
export default SubsystemViewAntDTable;
